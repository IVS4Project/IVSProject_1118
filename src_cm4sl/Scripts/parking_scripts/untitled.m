function opt_path = hybridAstar_main()
    ...
    p.yawResolution  = deg2rad(10); % [rad]
    p.stepSize       = 1.0;         % [m] arc length per motion primitive
    p.R_min          = 5.8;         % [m] 최소 회전반경
    % 차량/장애물 충돌 반경 (대략 값, 튜닝 가능)
    p.vehicleRadius  = 1.5;         % [m] ego 차량 반경 (원 모델)
    p.obsRadius      = 1.5;         % [m] 각 장애물 서클 반경
    ...
    opt_path = path; % [N x 4] : [x, y, yaw, dir(전진:1, 후진:-1)]
...
function [path, success] = hybridAstar_planner(start, goal, obstacles, space, p)
    ...
    % Pre-alloc
    costList    = inf(nx, ny, nyaw);   % g-value
    nodeIdList  = zeros(nx, ny, nyaw); % index of node in "nodes" array
    closedList  = false(nx, ny, nyaw); % closed set flag
...
function children = CreateNode(node, p)
    ...
    for i = 1:length(steeringSet)
        ...
        for d = 1:length(dirSet)
            ...
            [x_new, y_new, yaw_new] = BicycleKinematic(node.x, node.y, node.yaw, kappa, s);
...
function path = backtrackPath(nodes, goalId)
    ...
    while idx ~= 0
        n = nodes(idx);
        tmp = [n.x, n.y, n.yaw, n.dir; tmp]; %#ok<AGROW>
        idx = n.parent;
    end
    path = tmp;
end
